...
export function normalizePost(post) {
    ...
    return {
        ...
        excerpt,
        body,
        coverImage: safeUrl(post.coverImage),
...
export function renderBlogTags(tags, className = 'blog-card__tag') {
    return tags.map((tag) => `
        <span class="${escapeHtml(className)}">${escapeHtml(tag)}</span>
    `).join('');
}

export function renderBlogCard(post, index) {
    return `
        <article class="blog-card">
            ...
            <div class="blog-card__content">
                ...
                ${post.tags.length ? `
                    <div class="blog-card__tags" aria-label="${escapeHtml(post.title)} tags">
